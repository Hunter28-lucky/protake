...
final class PhoneNumberMatcher implements Iterator<PhoneNumberMatch> {
    ...
    private static String[] getNationalNumberGroups(PhoneNumberUtil phoneNumberUtil, Phonenumber.PhoneNumber phoneNumber, Phonemetadata.NumberFormat numberFormat) {
        if (numberFormat != null) {
            return phoneNumberUtil.formatNsnUsingPattern(phoneNumberUtil.getNationalSignificantNumber(phoneNumber), numberFormat, PhoneNumberUtil.PhoneNumberFormat.RFC3966).split(Constants.ACCEPT_TIME_SEPARATOR_SERVER);
        }
        ...
        return str.substring(str.indexOf(45) + 1, iIndexOf).split(Constants.ACCEPT_TIME_SEPARATOR_SERVER);
    }
