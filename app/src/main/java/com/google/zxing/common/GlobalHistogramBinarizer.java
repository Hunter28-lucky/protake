    ...
    public BitMatrix getBlackMatrix() throws NotFoundException {
        ...
        for (int i5 = 0; i5 < height; i5++) {
            ...
            for (int i7 = 0; i7 < width; i7++) {
                if ((matrix[i6 + i7] & ExifInterface.MARKER) < iEstimateBlackPoint) {
                    bitMatrix.set(i7, i5);
                }
            }
        }
    ...
    public BitArray getBlackRow(int i, BitArray bitArray) throws NotFoundException {
        ...
        if (width < 3) {
            for (int i4 = 0; i4 < width; i4++) {
                if ((row[i4] & ExifInterface.MARKER) < iEstimateBlackPoint) {
                    bitArray.set(i4);
                }
            }
        } else {
            int i5 = 1;
            int i6 = row[0] & ExifInterface.MARKER;
            int i7 = row[1] & ExifInterface.MARKER;
            while (i5 < width - 1) {
                int i8 = i5 + 1;
                int i9 = row[i8] & ExifInterface.MARKER;
