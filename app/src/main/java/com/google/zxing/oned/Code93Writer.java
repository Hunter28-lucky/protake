...
public class Code93Writer extends OneDimensionalCodeWriter {
    ...
    private static int computeChecksumIndex(String str, int i) {
        ...
        for (int length = str.length() - 1; length >= 0; length--) {
            iIndexOf += Code93Reader.ALPHABET_STRING.indexOf(str.charAt(length)) * i2;
            i2++;
            if (i2 > i) {
                i2 = 1;
            }
    ...
    public boolean[] encode(String str) {
        ...
        if (length <= 80) {
            ...
            toIntArray(iArr2[computeChecksumIndex(str + Code93Reader.ALPHABET_STRING.charAt(iComputeChecksumIndex), 15)], iArr);
