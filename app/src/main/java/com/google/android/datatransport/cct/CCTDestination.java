...
public final class CCTDestination implements EncodedDestination {
    ...
    public static String decodeExtras(@NonNull byte[] bArr) {
        return new String(bArr, Charset.forName(Platform.UTF_8));
    }
    ...
    public static CCTDestination fromByteArray(@NonNull byte[] bArr) {
        String str = new String(bArr, Charset.forName(Platform.UTF_8));
    ...
    public byte[] asByteArray() {
        ...
        objArr[3] = str;
        return String.format("%s%s%s%s", objArr).getBytes(Charset.forName(Platform.UTF_8));
    }
