    ...
    public List<UUID> extractUUIDs(byte[] bArr) {
        ...
        while (byteBufferOrder.remaining() > 2 && (b = byteBufferOrder.get()) != 0) {
            switch (byteBufferOrder.get()) {
                ...
                case 3:
                    while (b >= 2) {
                        arrayList.add(UUID.fromString(String.format(UUID_BASE, Short.valueOf(byteBufferOrder.getShort()))));
                        b = (byte) (b - 2);
                    }
                    continue;
                case 4:
                case 5:
                    while (b >= 4) {
                        arrayList.add(UUID.fromString(String.format(UUID_BASE, Integer.valueOf(byteBufferOrder.getInt()))));
                        b = (byte) (b - 4);
                    }
                    break;
                case 6:
