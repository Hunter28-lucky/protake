...
public class PAT {
    ...
    public int get(IBitstream iBitstream) throws IOException {
        ...
        this.total_pat_program = (this.section_length + (-9)) / 4;
        this.loop_cnt = 0;
        while (this.loop_cnt < this.total_pat_program) {
            Util.trace(iBitstream.getpos(), 0, 0, "processing PATProgram program");
            PATProgram pATProgram = new PATProgram();
            this.program = pATProgram;
            i += pATProgram.get(iBitstream);
            this.loop_cnt++;
        }
        this.CRC_32 = iBitstream.getbits(32);
        Util.trace(iBitstream.getpos() - 32, 32, this.CRC_32, "CRC_32 (" + this.CRC_32 + ")");
        Util.trace(iBitstream.getpos(), 0, 0, "end PAT");
        return i;
    }
    ...
    public int put(IBitstream iBitstream) throws IOException {
        ...
        this.loop_cnt = 0;
        while (this.loop_cnt < this.total_pat_program) {
            iPut += this.program.put(iBitstream);
            this.loop_cnt++;
        }
    ...
    public int putxml(IBitstream iBitstream, boolean z) throws IOException {
        ...
        while (this.loop_cnt < this.total_pat_program) {
            Util.trace(iBitstream.getpos(), 0, 0, "processing PATProgram program");
            this.program = new PATProgram();
            XML.IntoAClass("program", 0);
            iPutxml += this.program.putxml(iBitstream, z);
