...
public class AudioSampleEntry extends SampleEntry {
    ...
    public static void _setLabels(TrakBox trakBox, Label[] labelArr) {
        ChannelBox channelBoxCreateChannelBox = (ChannelBox) NodeBox.findFirstPath(trakBox, ChannelBox.class, new String[]{"mdia", "minf", "stbl", "stsd", null, "chan"});
        if (channelBoxCreateChannelBox == null) {
            channelBoxCreateChannelBox = ChannelBox.createChannelBox();
            ((SampleEntry) NodeBox.findFirstPath(trakBox, SampleEntry.class, new String[]{"mdia", "minf", "stbl", "stsd", null})).add(channelBoxCreateChannelBox);
        }
        setLabels(labelArr, channelBoxCreateChannelBox);
    }

    public static AudioSampleEntry audioSampleEntry(String str, int i, int i2, int i3, int i4, ByteOrder byteOrder) {
        AudioSampleEntry audioSampleEntryCreateAudioSampleEntry = createAudioSampleEntry(Header.createHeader(str, 0L), (short) i, (short) i3, (short) 16, i4, (short) 0, 0, 65535, 0, 1, i2, i3 * i2, i2, (short) 1);
        NodeBox nodeBox = new NodeBox(new Header("wave"));
        audioSampleEntryCreateAudioSampleEntry.add(nodeBox);
        nodeBox.add(FormatBox.createFormatBox(str));
        nodeBox.add(EndianBox.createEndianBox(byteOrder));
        nodeBox.add(Box.terminatorAtom());
        return audioSampleEntryCreateAudioSampleEntry;
    }
    ...
    public static Label[] getLabelsFromChan(ChannelBox channelBox) {
        ...
        ChannelLayout[] channelLayoutArrValues = ChannelLayout.values();
        while (i < channelLayoutArrValues.length) {
            ChannelLayout channelLayout2 = channelLayoutArrValues[i];
    ...
    public void parse(ByteBuffer byteBuffer) {
        ...
        this.sampleRate = Platform.unsignedInt(byteBuffer.getInt()) / 65536.0f;
        short s = this.version;
        if (s == 1) {
