...
public class TrakBox extends NodeBox {
    ...
    public Rational getPAR() {
        PixelAspectExt pixelAspectExt = (PixelAspectExt) NodeBox.findFirstPath(this, PixelAspectExt.class, new String[]{"mdia", "minf", "stbl", "stsd", null, "pasp"});
    ...
    public void setDataRef(String str) {
        ...
        UrlBox urlBoxCreateUrlBox = UrlBox.createUrlBox(str);
        if (dref == null) {
            DataRefBox dataRefBoxCreateDataRefBox = DataRefBox.createDataRefBox();
            dinf.add(dataRefBoxCreateDataRefBox);
            dataRefBoxCreateDataRefBox.add(urlBoxCreateUrlBox);
        } else {
            ListIterator<Box> listIterator = dref.boxes.listIterator();
            while (listIterator.hasNext()) {
                if ((((FullBox) listIterator.next()).getFlags() & 1) != 0) {
                    listIterator.set(urlBoxCreateUrlBox);
                }
            }
        }
    }
