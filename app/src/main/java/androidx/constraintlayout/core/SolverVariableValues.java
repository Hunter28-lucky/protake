...
public class SolverVariableValues implements ArrayRow.ArrayRowVariables {
    ...
    private void displayHash() {
        for (int i = 0; i < this.HASH_SIZE; i++) {
            if (this.keys[i] != -1) {
                ...
                boolean z = false;
                while (!z) {
                    str = str + MyUTIL.white_space + this.variables[i2];
    ...
    public void display() {
        ...
        for (int i2 = 0; i2 < i; i2++) {
            SolverVariable variable = getVariable(i2);
            if (variable != null) {
                System.out.print(variable + " = " + getVariableValue(i2) + MyUTIL.white_space);
            }
        }
        System.out.println(" }");
    }
    ...
    public String toString() {
        ...
        for (int i2 = 0; i2 < i; i2++) {
            SolverVariable variable = getVariable(i2);
            if (variable != null) {
                String str2 = str + variable + " = " + getVariableValue(i2) + MyUTIL.white_space;
                int iIndexOf = indexOf(variable);
                String str3 = str2 + "[p: ";
                String str4 = (this.previous[iIndexOf] != -1 ? str3 + this.mCache.mIndexedVariables[this.variables[this.previous[iIndexOf]]] : str3 + Constants.CP_NONE) + ", n: ";
                str = (this.next[iIndexOf] != -1 ? str4 + this.mCache.mIndexedVariables[this.variables[this.next[iIndexOf]]] : str4 + Constants.CP_NONE) + "]";
            }
        }
        return str + " }";
    }
